...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(1)))
        self.dropout = nn.Dropout(p=dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.hidden_size, self.n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.empty(1)))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.stack([torch.flatten(torch.index_select(self.embeddings, 0, w[i])) for i in range(w.shape[0])],dim=0)
    ...
    def forward(self, w):
        ...
        activate_f = nn.ReLU()
        hidden_layer = activate_f(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        hidden_layer = self.dropout(hidden_layer)
        logits = torch.matmul(hidden_layer, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
