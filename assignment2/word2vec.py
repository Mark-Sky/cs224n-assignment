...
def naiveSoftmaxLossAndGradient(
    ...
    y[outsideWordIdx] = 1
    gradCenterVec = outsideVectors.T @ (y_pred - y)
    gradOutsideVecs = np.multiply(centerWordVec, np.expand_dims(y_pred - y, axis = 1))
    loss = -(outsideVectors[outsideWordIdx].T @ centerWordVec) + np.log(np.sum(np.exp(outsideVectors @ centerWordVec)))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(outsideVectors[outsideWordIdx].T @ centerWordVec)) - np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)))
    posGrad = (sigmoid(outsideVectors[outsideWordIdx].T @ centerWordVec) - 1) * centerWordVec
    negGrad = np.expand_dims(sigmoid(outsideVectors[negSampleWordIndices] @ centerWordVec),axis =1) @ np.expand_dims(centerWordVec,axis = 0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(K):
        gradOutsideVecs[negSampleWordIndices[i]] += negGrad[i]
    gradOutsideVecs[outsideWordIdx] += posGrad
    gradCenterVec = (sigmoid(outsideVectors[outsideWordIdx].T @ centerWordVec) - 1) * outsideVectors[outsideWordIdx] +\
                    np.sum(np.expand_dims(sigmoid(outsideVectors[negSampleWordIndices] @ centerWordVec),axis=1) * outsideVectors[negSampleWordIndices],axis = 0)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in outsideWords:
        loss_tmp, gradCenterVec, gradOutsideVector = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[i],outsideVectors,dataset)
        loss += loss_tmp
